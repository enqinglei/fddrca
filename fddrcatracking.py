...
@app.route("/weather", methods=["GET"]) # team RCA status
def weather():
    if request.method == "GET":
        global gusername
        if gusername == 'directly':
            count = Todo.query.filter_by(user_id=g.user.id, IsRcaCompleted='No',\
                                         NoNeedDoRCAReason='').count()
            count1 = Todo.query.filter_by(user_id=g.user.id, \
                                         NoNeedDoRCAReason='',IsRcaCompleted='Yes').count()
            return jsonify(rcadone=count1, rcaundone=count, user=g.user.username)
        else:
            count = Todo.query.filter_by(Handler = gusername, IsRcaCompleted='No',NoNeedDoRCAReason='').count()
            count1 = Todo.query.filter_by(Handler = gusername, NoNeedDoRCAReason='',IsRcaCompleted='Yes').count()
            return jsonify(rcadone=count1, rcaundone=count, user= gusername)

month={'01':'Jan','02':'Feb','03':'Mar','04':'Apr','05':'May','06':'Jun','07':'Jul','08':'Aug','09':'Sep','10':'Oct','11':'Nov','12':'Dec'}
@app.route("/teamapstatus", methods=["GET"]) # team ap status
#@login_required
def teamapstatus():
    if request.method == "GET":
        data = []
        ...
        for k in keys:
            ...
            if gusername == 'directly':
                ...
                items = TodoAP.query.filter_by(QualityOwner=gusername).all()
                user = gusername
            for item in items:
    ...
    return jsonify(month = [x[0] for x in data],
                   ...
                   apundone=[x[3] for x in data],user=user)
...
def api_ap_upload():
    ...
    if f and allowed_file(f.filename):
        ...
        print filename
        f.save(filename)
        a,PRID = import_ap_fromexcel(filename)
        if a is True:
            flash('AP item have been successfully imported')
        else:
            flash('APs importing failed')
        newfilename = PRID + '.xlsm'
        fullpathnewfilename = os.path.join(file_dir, newfilename)
        shutil.copyfile(filename, fullpathnewfilename)
...
def UpdateToJira(PRID,RcaSubtaskJiraId,jira):
        ...
        if value == 'Update':
            ...
            if todo_item.Handler == username or username in admin:
                ...
                if jira == False:
                    flash('Connection with JIRA lost, Please redo login')
                    return redirect(url_for('logout'))
                UpdateToJira(PRID,JiraIssueId,jira)
            else:
                flash('You are not authorized to edit this item', 'error')
            return redirect(url_for('show_or_update', PRID=PRID, user=user))
